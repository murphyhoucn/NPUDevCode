...
// 每个线程处理两个元素
__global__ void SumArrayOnDevice2Elements(float *a, float *b, float *c, const int kN)
{
    int i = (blockIdx.x * blockDim.x + threadIdx.x) * 2; // 每个线程处理两个元素
    if (i < kN)
    {
        c[i] = a[i] + b[i];
        if (i + 1 < kN)
...
int main()
{
    ...
    // block.x /= 2;
    i_strat = CpuSecond();
    // SumArrayOnDevice2Elements<<<grid, block>>>(d_a, d_b, d_c, n_element);
...
/*
 * 每个线程处理一个元素
 * i = 0，1，2，3，4,....16777216
 * 线程idx = 0，1，2，3，4,....16777216
 */

/*
Using device 0: NVIDIA GeForce RTX 3080
Vector size 16777216
Execution configuration<<<65536, 256>>> Time eplapsed 0.000416 sec
Arrays match.
*/
 ...
 * 线程idx = 4,5
 * ....
 * i = 8388607 * 2 = 16777214
 * 线程idx = 16777214,16777215
 */

/*
Using device 0: NVIDIA GeForce RTX 3080
Vector size 16777216
Execution configuration<<<65536, 128>>> Time eplapsed 0.000416 sec
Arrays match.
 */